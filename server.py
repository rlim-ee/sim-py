# server.py — thème manuel (Clair/Sombre) + réactivité des couleurs Plotly
...
# Couleurs lignes/points (inchangées entre thèmes)
COLORS = {
    "consumption": "#1F6FEB",   # bleu
    "consumption_dark": "#58A6FF",
    "production":  "#2EA043",   # vert
...
def server(input, output, session):
    ...
    # Appliquer le thème (ajoute/retire la classe sur <html>)
    @reactive.Effect
    def _apply_theme():
        session.send_custom_message("set-theme", input.theme_mode())

    # Fonctions utilitaires couleurs/axes selon thème
    def _theme_text_color():
        return "#0B162C" if input.theme_mode() == "light" else "#F8FAFC"

    def _grid_color():
        return "rgba(15,22,44,.08)" if input.theme_mode() == "light" else "rgba(203,213,225,.28)"

    def _consumption_color():
        return COLORS["consumption"] if input.theme_mode() == "light" else COLORS["consumption_dark"]
    ...
    # Graphique 2 — Personnalisé (2 entrées; étendable)
