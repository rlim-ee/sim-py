...
def carte_flapd_server(input, output, session):
    ...
    def _(): selected_ville.set("Paris")

    @reactive.effect
    @reactive.event(input.go_london)
    def _(): selected_ville.set("London")

    @reactive.effect
    @reactive.event(input.go_amsterdam)
    def _(): selected_ville.set("Amsterdam")

    @reactive.effect
    @reactive.event(input.go_frankfurt)
    def _(): selected_ville.set("Frankfurt am Main")

    @reactive.effect
    @reactive.event(input.go_dublin)
    def _(): selected_ville.set("Dublin")

    @reactive.effect
    @reactive.event(input.reset_vue)
    def _(): selected_ville.set("All")
    ...
    def map_flapd_sites():
        ...
        # === Points ===
        for _, r in df.iterrows():
            cap = float(r["capacity_e"]) if pd.notna(r["capacity_e"]) else 0
            ar = float(r["area_m2"]) if pd.notna(r["area_m2"]) else 10
    ...
    def encarts_villes():
        ...
        # tableau triable + légende pastilles
        table_html = f"""
        <style>
        th.sortable {{
            cursor: pointer;
            ...
            <thead>
                <tr>
                    <th onclick="sortTable(0)" class="sortable">
                        Hub <span class="sort-icon"></span>
                    </th>
                    <th onclick="sortTable(1)" class="sortable">
                        Nombre de DC recensés<span class="sort-icon"></span>
                    </th>
                    <th onclick="sortTable(2)" class="sortable">
                        Surface moyenne (m²) <span class="sort-icon"></span>
                    </th>
                    <th onclick="sortTable(3)" class="sortable">
                        Surface totale (m²) <span class="sort-icon"></span>
                    </th>
                    <th onclick="sortTable(4)" class="sortable">
                        Puissance moyenne (MW) <span class="sort-icon"></span>
                    </th>
                    <th onclick="sortTable(5)" class="sortable">
                        Puissance médiane (MW) <span class="sort-icon"></span>
                    </th>
                    <th onclick="sortTable(6)" class="sortable">
                        PUE moyen <span class="sort-icon"></span>
                    </th>
                </tr>
            </thead>
            <tbody>
        ...
        <div style="text-align:center; margin-top:8px; font-size:13px; color:#666;">
            Données collectées depuis <a href="https://www.datacentermap.com/" target="_blank" rel="noopener noreferrer">
        <b>DataCenterMap</b></a> (avril 2025).
        </div>
        """
...
def server(input, output, session, app_dir):
    return carte_flapd_server(input, output, session)